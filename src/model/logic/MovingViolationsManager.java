...
public class MovingViolationsManager implements IMovingViolationsManager {
	...
	public model.data_structures.Queue<Integer>  ConsultarInfraccionesPromedioRango(String min, String max) {
		LinkedList<Integer> violationCode =new LinkedList<Integer>();
		...
		for (int i = 0; i < listaViolaciones.size(); i++) {
			...
			if(violationCode.size()==0&&FINEprom.size()==0) {
				violationCode.add(Integer.parseInt(este.getViolationCode()));
				FINEprom.add(Integer.parseInt(este.getFine()));
				aux.add(1);
			}
			else {
				boolean hecho=false;
				for(int e=0;e<violationCode.size();e++) {
					if((Integer.parseInt(este.getViolationCode())==violationCode.get(e))) {
						int prom=FINEprom.get(e)+Integer.parseInt(este.getFine());
						FINEprom.deleteNodeAtIndex(e);
						FINEprom.add(prom,e);
						//Conteo auxiliar para el promedio.
						int auxx=aux.get(e)+1;
						aux.deleteNodeAtIndex(e);
						aux.add(auxx,e);
						hecho=true;
					}
				}
		...
		//Paso por encima de los valores haciendo promedio
		for (int i = 0; i < FINEprom.size(); i++) {
			int calculo=FINEprom.get(i)/aux.get(i);
			FINEprom.deleteNodeAtIndex(i);
			FINEprom.add(calculo, i);
		}
		//se agregan a una Queue q. FORMATO: primer elemento=cantidad de elementos=n. Continúa n cantidad
//		de elementos siendo los violationCode; cuando esto termina siguen los FINEprom.
		model.data_structures.Queue<Integer> q=new model.data_structures.Queue<Integer>();
		q.enqueue(violationCode.size());
		for (int i = 0; i < violationCode.size(); i++) {
			q.enqueue(violationCode.get(i));
		}
		for (int i = 0; i < FINEprom.size(); i++) {
			q.enqueue(FINEprom.get(i));
		}
	...
	public Stack ConsultarInfraccionesPagadoRango(int min2, int max2, String orden) {
		...
		//------ordenarlo-----
		//--------------------
//		LinkedList<VOMovingViolations> aux = new LinkedList<>();
//		for(VOMovingViolations esta:infrac) {
//			if(aux.size()==0) aux.add(esta);
//			else {
//				for (int i = 0; i < aux.size(); i++) {
//					int comp=esta.getTicketIssue().compareTo(aux.get(i).getTicketIssue());
//					
//				}
//			}
//		}
